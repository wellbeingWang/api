 ...
 */
@Component
public class CLTBKJsonHandler extends CltApiURLHandler{
	...
	public String getRedirectURL(URLHandlerContext context, ClickHandlerInfo handlerInfo) {
		...
		try {
			...
			//获取后台配置的升级信息
			Map<String, String> dbInfo = baseRedisService.getMap(CacheConstant.CLT_BK_JSON + productId);
			//获取客户端传递参数
			//版本信息
			...
			if(isHave(dbInfo.get("version"), version)
				...
				char[] file = Base64.encode(dbInfo.get("file").getBytes());
				String fi = new String(file);
				context.setStringInfo(fi);
