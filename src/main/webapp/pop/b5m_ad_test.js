...
function b5m_FillSlot(index) {
    ...
    if (index < b5m_ADS.length && b5m_ADS[index][0] != undefined) {
        ...
        if(b5m_ADS[index].length>1 && b5m_ADS[index]==28){
            ...
            iframe.setAttribute("id", "xxx");
            var scr = document.getElementById("b5m_union_ad");
            scr.parentNode.appendChild(iframe);
            var h = '<!DOCTYPE html><html lang="en"><head><meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"><meta charset="utf-8" /><title>AD</title><style>*{margin:0;padding:0}</style><script>function callback(){var p=window.parent,ifr=p.document.getElementById("' + f.id + '");p.__corpize_callback_' + c.id + '("' + a + "|" + f.id + '");ifr.parentNode.removeChild(ifr)}</script></head><body><div id="logo" style="position:absolute;z-index:2147483647;left:0;top:0;width:33px;height:18px;overflow:hidden;"><a href="http://www.fanmila.com" target="_blank" style="display:block;"><img style="position:absolute;left:0;top:0;z-index:1;border:0;" src="http://ssp.corpize.com/img/gg.png" width="22" /></a></div>' + b5m_ADS[index][0] + "</body></html>"
            try {
                document.frames[iframe.id].document.write(h)
            } catch (i) {}
